...
export class AuthService {
  ...
  async localLogin(loginUserInput: LoginUserInput) {
    ...
    if (user) {
      owner = user;
      password = await this.usersService.findPasswordByEmailUsingQueryBuilder(
        loginUserInput.email,
      );
      type = UserType.USER;
    } else if (trainer) {
  ...
  async googleLogin(req, res) {
    ...
    const refreshToken = (
      await this.refreshTokensService.createRefreshToken({
        targetId: owner.id,
        targetType: owner instanceof User ? UserType.USER : UserType.TRAINER,
  ...
  async renewToken(
    ...
    const refreshTokenFromDB =
      await this.refreshTokensService.findOneByRefreshToken(body.refreshToken);
